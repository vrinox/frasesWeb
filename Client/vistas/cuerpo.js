...
Construc.prototype.construirInicio = function() {
	...
	jarvis.usarLib('Chat').then(function(){
		...
		hijos:[
			{
				...
				hijos:[],
				click: function(){
					jarvis.usarLib('Opciones').then(function(lib){
						//agrego el operador a la libreria
						lib.op = new Settings();
						//construyo las opciones
						var opciones = lib.op.construirOpciones();
						return torque.Operacion({
							operacion:'datosPer',
							...
							formulario.asignarValores(res.usuario);
...
Construc.prototype.buscarContactos = function(){
	jarvis.usarLib('Contactos').then(function(){
